...
# Services intended to be usable from the LAN immediately after the wizard.
# The probe is executed from the Windows PC running the wizard, so an OK result
# proves that the published port is reachable from the user's LAN, not merely
# listening inside Docker.
LOCAL_SERVICES = [
...
class Wizard(current.Wizard):
    """Final wizard layer: prove that the deployed stack is reachable on LAN."""
    ...
    def probe_local_services(self) -> dict[int, bool]:
        ...
        # docker compose up -d can return while some web listeners are still
        # finishing their startup. Probe in parallel and retry briefly.
    ...
    def build_complete(self) -> None:
        # Keep the dedicated offline Demo/Dry Run completion screen unchanged.
        ...
        results = self.probe_local_services() if started else {
            port: False for _, port, _ in LOCAL_SERVICES
        }
        ...
        if started and ok_count == total:
            ...
            subtitle = (
                f"{ok_count}/{total} servizi LAN raggiungibili da questo PC. "
                "Lo stack è già avviato e utilizzabile in locale."
            )
            border = "#1E5B46"
            ...
            subtitle = (
                f"{ok_count}/{total} porte LAN rispondono. I servizi in rosso possono essere ancora "
                "in avvio oppure richiedere una correzione prima di considerare il setup concluso."
            )
            border = "#7A5714"
            ...
            subtitle = (
                "Hai disattivato 'Avvia stack completo'. I file sono configurati, ma i servizi LAN "
                "non possono essere verificati finché non esegui docker compose --profile all up -d."
            )
            border = "#7A5714"
            heading_color = self.WARNING
        ...
        ctk.CTkLabel(
            ...
            anchor="w",
        ).grid(row=0, column=0, columnspan=4, sticky="ew", padx=18, pady=(16, 3))
        ctk.CTkLabel(
            body,
            text=subtitle,
            text_color="#AFC0D9",
            ...
            justify="left",
            wraplength=980,
        ).grid(row=1, column=0, columnspan=4, sticky="ew", padx=18, pady=(0, 12))
        ...
        for row, (label, port, scheme) in enumerate(LOCAL_SERVICES, start=2):
            ...
            ).grid(row=row, column=0, sticky="w", padx=(18, 6), pady=4)
            ctk.CTkLabel(
                body,
                text=label,
                width=170,
            ...
            ).grid(row=row, column=3, padx=(8, 18), pady=4)

        ctk.CTkLabel(
            body,
            text=(
                "Nota: AdGuard Home è volutamente indicato sulla porta 3010 perché il suo wizard iniziale "
                "resta un passaggio applicativo manuale. Un servizio può essere raggiungibile anche se la "
                "sua configurazione interna non è ancora stata completata."
            ),
            text_color="#8294AE",
            anchor="w",
            justify="left",
